...
import { RunNotFoundError, TenantNotFoundError, TenantCreationError } from "@hubble/core"

// Re-export for backward compatibility
export { RunNotFoundError, TenantNotFoundError, TenantCreationError } from "@hubble/core"
...
export async function upsertTenantDestination(
  ...
  const db = createServiceClient()
  const { error } = await db.from("data_destinations").upsert(
    {
      org_id: orgId,
      md_db_name: mdDbName,
