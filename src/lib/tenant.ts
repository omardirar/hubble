 ...
 */
export async function resolveDbForOrg(orgId: string, clientHint?: string): Promise<string> {
  const allowed = /^[a-zA-Z0-9_-]{1,128}$/
  if (!allowed.test(orgId)) {
    throw new AuthorizationError("Invalid org id format")
  }
  const db = orgId // use the raw Clerk orgId as the db name

  if (clientHint && clientHint !== db) {
    throw new AuthorizationError("Client-provided db hint does not match authorized database")
  }
